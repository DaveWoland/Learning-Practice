...
def rozbor_vety(dictionary,text, order, character):
    ...
    dictionary.setdefault('Text'+str(order), {
                                                ...
                                                f"List_of_words_started_with_'{character}'": [slovo for slovo in lst if slovo.lower().startswith(character)],
...
def slova_na_pismeno(lst, letter):
    lst0 = [slovo for slovo in lst if slovo.lower().startswith(letter)]
    return lst0
